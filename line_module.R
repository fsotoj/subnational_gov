# Modernized Plotly line chart module (refactored for external legend placement)
# - Dark styling, formatted y-axis ticks (discrete/continuous/percentage)
# - External legend now has its own UI helper so you can place it ANYWHERE in the main UI
  ...
  tagList(
    ...
    div(
      ...
      tags$style(HTML(sprintf('
        ...
        #%1$s-legend .legend-country { display: inline-flex; align-items: center; gap: 8px; color: #E5E7EB; font-weight: 600; font-size: 12px; opacity: 0.95; }
        #%1$s-legend .country-swatch { width: 10px; height: 10px; border-radius: 2px; display: inline-block; box-shadow: 0 0 0 1px #111; }
      ...
      ', ns("line_plot")))),
      div(id = paste0(ns("line_plot"), "-legend"), class = "legend-box",
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$line_plot <- renderPlotly({
      ...
      df <- data() %>%
        ...
        mutate(date_year = suppressWarnings(as.Date(paste0(year, "-01-01")))) %>%
        drop_na(date_year)
      req(nrow(df) > 0)
      
      # Pretty label and type from dict
      pretty_name <- dict %>% filter(variable == input_variable()) %>% pull(pretty_name) %>%
        { if (length(.) == 0 || is.na(.)) input_variable() else . }
      var_type <- dict %>% filter(variable == input_variable()) %>% pull(type) %>%
      ...
      for (state in selected_states) {
        ...
        color <- country_colors[[country]] %||% "#60A5FA"
        df_state <- df_state %>% mutate(.formatted_value = format_value(value, var_type))
      ...
      # Dark theme -------------------------------------------------------------
      dark_bg <- "#0B1220"    # canvas
      dark_paper <- "#0E1626" # surrounding
      grid_col <- "#1F2937"
      axis_txt <- "#D1D5DB"
      accent <- "#93C5FD"
      ...
      fig <- fig %>% layout(
        ...
        hovermode = "x unified",
        hoverlabel = list(bgcolor = dark_paper, bordercolor = accent, font = list(color = axis_txt, size = 12)),
        showlegend = FALSE,
      ...
      fig %>% config(
        displaylogo = FALSE,
        modeBarButtonsToRemove = c("autoScale2d", "toggleSpikelines", "lasso2d", "select2d", "zoomIn2d", "zoomOut2d", "resetScale2d"),
        toImageButtonOptions = list(format = "png", filename = paste0(gsub("[^A-Za-z0-9_]", "_", pretty_name), "_lineplot"), width = 1400, height = 800, scale = 1)
      )
    })
    ...
    output$legend <- renderUI({
      ...
      df <- data() %>% filter(state_name %in% input_states())
      ...
      order_df <- df %>% mutate(state_order = match(state_name, selected_states)) %>% arrange(state_order)
      country_order <- unique(order_df$country_name)
      
      groups_ui <- lapply(country_order, function(ctry) {
        col <- country_colors[[ctry]] %||% "#60A5FA"
        states_ctry <- selected_states[selected_states %in% (df %>% filter(country_name == ctry) %>% pull(state_name) %>% unique())]
